...
describe("csvParser", () => {
  ...
  it("throws when Field API Name does not end with __c", () => {
    ...
    try {
      csvParser(filePath);
      throw new Error("Expected csvParser to throw");
    } catch (err) {
      // overall validation header
      expect(err.message).toMatch(/^CSV Validation Failed/);
      // specific row‐level error in err.details array
      expect(err.details).toContain(
        "Row 2: 'Field API Name' must end with '__c'."
      );
    }
  });
  ...
  it("throws when picklist type is missing options", () => {
    ...
    try {
      ...
      expect(err.message).toMatch(/^CSV Validation Failed/);
      expect(err.details).toContain(
        "Row 2: 'Picklist Options' must be provided for picklist fields."
      );
    }
  });
